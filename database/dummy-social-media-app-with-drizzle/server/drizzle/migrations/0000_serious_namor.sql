...
--> statement-breakpoint
CREATE TABLE `verify_password_table` (
	`id` int AUTO_INCREMENT NOT NULL,
	`user_id` int NOT NULL,
	`code` varchar(8) NOT NULL,
	`valid` boolean NOT NULL DEFAULT true,
	...
	CONSTRAINT `verify_password_table_id` PRIMARY KEY(`id`)
);
--> statement-breakpoint
...
ALTER TABLE `verify_emails_table` ADD CONSTRAINT `verify_emails_table_user_id_users_table_id_fk` FOREIGN KEY (`user_id`) REFERENCES `users_table`(`id`) ON DELETE cascade ON UPDATE no action;--> statement-breakpoint
ALTER TABLE `verify_password_table` ADD CONSTRAINT `verify_password_table_user_id_users_table_id_fk` FOREIGN KEY (`user_id`) REFERENCES `users_table`(`id`) ON DELETE cascade ON UPDATE no action;