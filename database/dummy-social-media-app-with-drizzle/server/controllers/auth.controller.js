...
export const login = async (req, res) => {
  ...
  try {
    ...
    const userData = await authServices.getUserDataByUserName(userName);
    ...
    const session = await authServices.createNewSession({
      userId: userData.id,
...
export const logout = async (req, res) => {
  if (!req.user) return res.redirect("/login");
  try {
    const userData = await authServices.getUserDataByUserName(
      req.user.userName
    );
    ...
    const sessionData = await authServices.getSessionDataBySessionId(
      req.user.sessionId
    );
    ...
    await authServices.deleteSessionDataBySessionId(req.user.sessionId);
...
export const getVerifyEmailToken = async (req, res) => {
  ...
  try {
    ...
    const insertedToken = await authServices.insertVerifyEmailTokenData({
...
export const verifyEmail = async (req, res) => {
  ...
  try {
    ...
    const userData = await authServices.getUserDataByEmail(email);
    if (!userData) {
      await authServices.deleteCodeDataById(codeData.id);
      return res.redirect("/login");
    }

    const codeData = await authServices.getCodeDataByCodeAndUserId(
      code,
      userData.id
    );
    ...
    await authServices.changeVerifyStatusByUserId(codeData.userId, codeData.id);

    const updatedUserData = await authServices.getUserDataByEmail(
      userData.email
    );
    await setUpAuthCookies(res, {
      ...updatedUserData,
